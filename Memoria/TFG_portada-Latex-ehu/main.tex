...
\begin{figure}[htbp]
    ...
    \caption{\small Imagen de un \lightit{pinhole grid} real hecha con xxxx.}
...
\begin{figure}[htbp]
    ...
    \caption{\small Imagen de un \lightit{pinhole grid} simulada.}
...
Tamaño de píxel \lightit{px} & \(5.5\ \mu\text{m}\) \\
Dimensión del sensor \text{\lightit{sendim}} & \(3072 \times 4096\ \text{px}\) \\
Número de \lightit{pinhole}-s \(n_{ph}\) & \(20 \times 15\) \\
Espaciado entre \lightit{pinhole}-s \lightit{spacing} & \(172\ \text{px}\) \\
Centro de la placa \(O\) & \([1536,\ 2048]\ \text{px}\) \\
Centro óptico \(C_o\) & \(O + [0,\ 258]\ \text{px}\) \\
Intensidad de \lightit{pinhole} \(I_i\) & \(3000\) \\
...
Centro de la placa \(O\) & \([1536,\ 2048]\ \text{px}\) \\
Centro óptico \(C_o\) & \(O + [0,\ 258]\ \text{px}\) \\
Intensidad de \lightit{pinhole} \(I_i\) & \(\in [0,3000]\) \\
Ángulo de rotación \(\theta\) & \(0\) \\
\hline
\end{tabular}
\end{table}
...
Es apreciable que las imágenes no han sido realizadas controlando los parámetros variables de \ \ref{subsec:simulaciones}. La placa no está centrada (el centro se diferencia con unos \lightit{pinhole}-s en forma de cruz) ni recta (hay algo de rotación), no se ha alineado previamente con el eje óptico y el centro óptico es una incognita, no todos los \lightit{pinhole}-s son lo suficientemente brillantes y tampoco se ha medido con precisión el \lightit{spacing}. Es decir, esta imagen entra en el marco de simulaciones 8 y los resultados tendrán, probablemente, los errores mostrados en la Tabla\ \ref{tab:resultados8}.\\

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.75\linewidth]{Imagenes/flat_centro_optico.png}
    \caption{\small Ejemplo de \lightit{Flat Frame} con su ajuste de círculo dibujado en azul y el centro óptico en verde.}
    \label{fig:flat}
\end{figure}

Para dar una estimación del centro óptico se ha propone utilizar el \lightit{Flat Frame}. Esto es una imagen de calibración utilizada en astrofotografía para corregir irregularidades ópticas como el viñeteo (esquinas oscuras), manchas de polvo en el sensor o filtro, y desalineaciones del campo. Se obtienen tomando una foto de una fuente de luz uniforme, permitiendo al software de procesado eliminar estos defectos. Debido a su simetría radial, es posible ajustar un círculo y usar el centro de este como centro óptico. En la Figura\ \ref{fig:flat} se puede observar un ejemplo de \lightit{Flat Frame} con su correspondiente ajuste y centro óptico marcado. Esto es simplemente una proposición y no se ha implementado en este caso concreto debido a la falta del \lightit{Flat Frame}. En cambio, se ha recurrido a la siguiente opción que es suponer como centro óptico el centro de la imagen que. Esta es una muy pobre aproximación y puede alterar notoriamente los resultados. Aunque, ante la falta de un \lightit{Flat Frame} se puede utilizar como opción de último recurso.\\
